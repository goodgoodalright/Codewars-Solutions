...
function arrayPacking(a) {
    ...
    for (let i = 0; i < a.length; i++) {
      let converted = (a[i]%2).toString(2)
      for (; a[i] > 1;) {
        a[i] = parseInt(a[i]/2)
        converted = (a[i]%2) + converted
      }
      if (converted.length < 8) {
        let rest = 8 - converted.length
        do {
          converted = '0' + converted
          rest--
        } while (rest > 0)
      }
      binaryArray.unshift(converted)
      converted = ''
    }
...
// DESCRIPTION:
// Task: you are given an array of up to four non-negative integers, each less than 256.
